...
(function(win, doc, $) {
    ...
    KinerLottery.prototype.init = function() {
        ...
        //点击抽奖 
        this.run=function(btnObj){
        	$(".dom_menu a").removeClass("btn");
            $(btnObj).find(".onnumber").hide();
            $(btnObj).find(".onstart").show();
            if ($(btnObj).hasClass('start') && !self.doing) {
                ...
                var key = $(btnObj).hasClass('no-start') ? "noStart" : $(btnObj).hasClass('completed') ? "completed" : "illegal";
        ...
        $('.KinerLotteryBtn').live('touchstart', function() {
        	self.run(this);
        });
